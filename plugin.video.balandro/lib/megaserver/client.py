# -*- coding: utf-8 -*-
import base64, json, random, struct, time, urllib2
...
class Client(object):
    ...
    def get_play_list(self):
        if len(self.files) > 1:
            ...
            return "http://" + self.ip + ":" + str(self.port) + "/" + urllib2.quote(self.files[0].name.encode("utf8"))
    ...
    def aes_cbc_decrypt(self, data, key):
      try:
        ...
        logger.info("MEGA con Cryptodome.Cipher")
      except:
        try:
          from Crypto.Cipher import AES
          decryptor = AES.new(key, AES.MODE_CBC, '\0' * 16)
          logger.info("MEGA con Crypto.Cipher")
        except:
          logger.info("MEGA no posible, falta criptografía")
          raise Exception("Se necesitan módulos de criptografía no presentes en este sistema")
