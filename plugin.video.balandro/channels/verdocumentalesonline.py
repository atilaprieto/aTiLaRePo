...
def mainlist(item):
    ...
    itemlist.append(item.clone( title='Últimos documentales', action='list_all' ))

    itemlist.append(item.clone( title='Listas de reproducción', action='list_playlists' ))
...
def list_playlists(item):
    ...
    try:
        ...
        for vid in data['list']:
            ...
            title = '%s [COLOR red](%d vídeos)[/COLOR]' % (vid['name'], vid['videos_total'])
...
def list_all(item):
    ...
    try:
        ...
        for vid in data['list']:
            ...
            title = '[COLOR red](%s)[/COLOR] %s' % (config.format_seconds_to_duration(vid['duration']), vid['title'])
            plot = '' if not vid['description'] else scrapertools.htmlclean(vid['description'])
        ...
        if data['has_more']:
            itemlist.append(item.clone( title='>> Página siguiente', action='list_all', page = item.page + 1 ))
    except:
        pass
...
def findvideos(item):
    ...
    itemlist.append(Item( channel = item.channel, action = 'play', server='dailymotion', 
                          title = '', url = item.url, language = 'Esp'
                   ))
