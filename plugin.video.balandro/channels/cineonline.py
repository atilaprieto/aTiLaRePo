...
def puntuar_calidad(txt):
    orden = ['Cam', 'TS-Screener', 'BR-Screener', 'DVD-Rip', 'HD', 'HD 720p', 'HD 1080p']
...
def findvideos(item):
    ...
    # Vídeos incrustados
    matches = re.compile('<div style="display: none" id="plays-([^"]+)">\s*(.*?)</div>', re.DOTALL).findall(data)
    for numero, enlace in matches:
        if not enlace: continue
        url = host + 'ecrypt?nombre=' + enlace
        ...
        if not lang: lang = scrapertools.find_single_match(data, ' href="#player2%s">([^<]+)</a></li>' % numero)
...
def play(item):
    ...
    if 'ecrypt?nombre=' in item.url:
        ...
        data = httptools.downloadpage(partes[0], post=urllib.urlencode( {'nombre': partes[1]})).data
        # ~ logger.debug(data)
        ...
        if not url: url = scrapertools.find_single_match(data, 'window.location="([^"]+)')
        # ~ logger.info(url)
