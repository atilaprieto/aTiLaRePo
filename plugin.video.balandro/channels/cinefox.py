...
def search(item, texto):
    ...
    try:
        ...
        if item.search_type == 'movie':
            item.url = host + '/search/peliculas/' + texto.replace(' ', '-')
        elif item.search_type == 'tvshow':
            item.url = host + '/search/series/' + texto.replace(' ', '-')
        else:
            item.url = host + '/search?q=%s' % texto.replace(' ', '+')
...
def busqueda(item):
    ...
    if next_page != "" and len(bloque) > 0:
        ...
        itemlist.append(item.clone(title=">> Página siguiente", url=next_page, action='busqueda'))
...
def secciones(item):
    ...
    if item.extra == 'peliculas' and item.seccion == 'Género':
        ...
        if not descartar_xxx:
            itemlist.append(item.clone(title='X (adultos +18)', url=host + '/catalogue?type=adultos', action='peliculas'))

    return sorted(itemlist, key=lambda it: it.title, reverse = True if item.seccion == 'Año' else False)
