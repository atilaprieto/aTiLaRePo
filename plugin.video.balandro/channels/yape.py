...
# Filtros para generos, idiomas, calidades y ordenes.
web_generos = [
   ['accion', 'Acci\xc3\xb3n'],
   ['animacion', 'Animaci\xc3\xb3n'],
   ['aventura', 'Aventura'],
   ['belico', 'B\xc3\xa9lico'],
   ['biografico', 'Biogr\xc3\xa1fico'],
   ['ciencia-ficcion', 'Ciencia Ficci\xc3\xb3n'],
   ...
   ['drama', 'Drama'],
   ['familia', 'Familia'],
   ['fantasia', 'Fantas\xc3\xada'],
   ['gore', 'Gore'],
   ['historico', 'Hist\xc3\xb3rico'],
   ['intriga', 'Intriga'],
   ['kids', 'Kids'],
   ['misterio', 'Misterio'],
   ['musical', 'Musical'],
...
web_ordenes = [
   ['mosts-today', 'Popular Hoy'],
   ['mosts-week', 'Popular esta semana'],
   ['latest', 'Nuevo'], ['time_update', 'Actualizadas']
   ]
...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone( title = 'Catálogo', action = 'list_all', url = host_catalogue + '?sort=latest' ))
    itemlist.append(item.clone( title = 'Más populares', action = 'list_all', url = host_catalogue ))
    ...
    if filtro != '':
        ...
        itemlist.append(item.clone( title = 'Listar con los filtros personalizados', action = 'filtrado', plot=plot, text_color='cyan' ))

        itemlist.append(item.clone( title = 'Modificar filtros a usar ...', action = 'modificar_filtro', plot=plot, folder=False, text_color='plum' ))
        itemlist.append(item.clone( title = 'Anular los filtros a usar ...', action = 'anular_filtro', plot=plot, folder=False, text_color='red' ))
    else:
...
def anular_filtro(item):
    config.set_setting('filtro', '', item.channel)
