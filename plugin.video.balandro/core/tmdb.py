...
import ast, copy, re, sqlite3, time
...
#
#   Incluyen:
#       set_infoLabels(source, seekTmdb, idioma_busqueda): Obtiene y fija (item.infoLabels) los datos extras de una o varias series, capitulos o peliculas.
#       set_infoLabels_item(item, seekTmdb, idioma_busqueda): Obtiene y fija (item.infoLabels) los datos extras de una serie, capitulo o pelicula.
#       set_infoLabels_itemlist(item_list, seekTmdb, idioma_busqueda): Obtiene y fija (item.infoLabels) los datos extras de una lista de series, capitulos o peliculas.
...
#           Antes de llamar al metodo set_infoLabels el titulo a buscar debe estar en item.fulltitle o en item.contentTitle y el año en item.infoLabels['year'].
...
#           IMDB (en item.infoLabels['IMDBNumber'] o item.infoLabels['code'] o item.infoLabels['imdb_id']), TVDB (solo series, en item.infoLabels['tvdb_id']), Freebase (solo series, en item.infoLabels['freebase_mid']),TVRage (solo series, en item.infoLabels['tvrage_id'])
...
def cache_response(fn):
    ...
    def wrapper(*args):
        ...
        def check_expired(ts, cache_expire=0):
            import datetime
        ...
        if not config.get_setting("tmdb_cache", default=False):
                ...
                # Para listas, fijar caché en 1 día para que no salgan siempre las mismas
                if '/popular?' in args[0] or '/top_rated?' in args[0] or '/now_playing?' in args[0] or '/on_the_air?' in args[0]: cache_expire = 0

                # ~ url_base64 = base64.b64encode(args[0])
                url = args[0]
                if PY3: url = str.encode(url)
                # ~ url = str(url.replace('https:', 'http:')) # No en Balandro, mantener mismo caché con los cambios de http: por https:
                ...
                if row and check_expired(float(row[1]), cache_expire):
...
def set_infoLabels_item(item, seekTmdb=True, idioma_busqueda='es', lock=None):
    ...
    def __leer_datos(otmdb_aux):
        # ~ logger.debug('item pre __leer_datos %s' % item.tostring())
        ...
        # ~ logger.debug('item post __leer_datos %s' % item.tostring())
    if seekTmdb:
        ...
        texto_alternativo = item.contentTitleAlt # Título de peli/serie alternativo por si falla el principal

        # Obtener i verificar season/episode válidos
        if item.infoLabels['season']:
            try:
                numtemporada = int(item.infoLabels['season'])
            except ValueError:
                logger.debug("El número de temporada no es válido")
                return -1 * len(item.infoLabels)

            if item.infoLabels['episode']:
                try:
                    numepisodio = int(item.infoLabels['episode'])
                except ValueError:
                    logger.debug("El número de episodio (%s) no es válido" % repr(item.infoLabels['episode']))
                    return -1 * len(item.infoLabels)
        ...
        if item.infoLabels['season']:
            ...
            if not otmdb_global or (item.infoLabels['tmdb_id'] and str(otmdb_global.result.get("id")) != item.infoLabels['tmdb_id']) \
                ...
                if item.infoLabels['tmdb_id']:
                    ...
                    otmdb_global = Tmdb(texto_buscado=item.infoLabels['tvshowtitle'], texto_alternativo=texto_alternativo, tipo=tipo_busqueda,
                                        idioma_busqueda=idioma_busqueda, year=item.infoLabels['year'])

            temporada = None; episodio = None
            if otmdb_global is not None: 
                __leer_datos(otmdb_global)
                if item.infoLabels['episode']:
                    episodio = otmdb_global.get_episodio(numtemporada, numepisodio)
                else:
            ...
            if item.infoLabels['episode']:
                # Tenemos numero de temporada y numero de episodio validos... rellenar datos episodio
                ...
                # Tenemos numero de temporada valido pero no numero de episodio... rellenar datos temporada
                item.infoLabels['mediatype'] = 'season'
                if temporada:
                    # ~ logger.debug(temporada)
                    if 'name' in temporada and temporada['name']:
        ...
        # Buscar...
            ...
            if otmdb is None:
                # No se ha podido buscar por ID... hacerlo por titulo
                if tipo_busqueda == 'tv':
                    # Busqueda de serie por titulo y filtrando sus resultados si es necesario
                    otmdb = Tmdb(texto_buscado=item.infoLabels['tvshowtitle'], texto_alternativo=texto_alternativo,
                                 tipo=tipo_busqueda, idioma_busqueda=idioma_busqueda, 
                                 filtro=item.infoLabels.get('filtro', {}), year=item.infoLabels['year'])
                else:
                    # Busqueda de pelicula por titulo y año o filtro...
                    if item.infoLabels['year'] or item.infoLabels['filtro']:
                        titulo_buscado = item.contentTitle if item.contentTitle else item.fulltitle

                        otmdb = Tmdb(texto_buscado=titulo_buscado, texto_alternativo=texto_alternativo,
                                     tipo=tipo_busqueda, idioma_busqueda=idioma_busqueda, 
                                     filtro=item.infoLabels.get('filtro', {}), year=item.infoLabels['year'])
...
# Para una peli/serie volver a buscarla en Tmdb con interacción del usuario para escoger.
# Devuelve True si se ha modificado el item y False en caso contrario.
def dialog_find_and_set_infoLabels(item):
    global otmdb_global

    if item.contentType == 'movie':
        tipo_busqueda = "movie"
        texto_busqueda = item.contentTitle
    else:
        tipo_busqueda = "tv"
        texto_busqueda = item.contentSerieName

    # Bucle hasta seleccionar un resultado o cancelar
    tmdb_result = None
    pedir_nuevo_texto = True
    while pedir_nuevo_texto:

        otmdb_global = Tmdb(texto_buscado=texto_busqueda, tipo=tipo_busqueda) # , idioma_busqueda='es'
        results = otmdb_global.get_list_resultados()

        if len(results) > 0: # Diálogo para escoger entre múltiples resultados
            ret = _seleccionar_un_resultado(results, tipo_busqueda)
            if ret > -1: # Si no ha cancelado guardar el resultado escogido y salir del bucle
                tmdb_result = results[ret]
        ...
        if pedir_nuevo_texto:
            ...
            if nuevo_texto is None or nuevo_texto == '' or nuevo_texto == texto_busqueda:
                ...
                texto_busqueda = nuevo_texto

    if tmdb_result is None:
        return False # Salir sin cambios

    # Asignar nuevo infoLabels con tmdb_id fijado y el nuevo título. Resetear thumbnail/fanart para no conservar los anteriores si el nuevo no los tiene
    logger.debug(item)
    item.infoLabels = InfoLabels()
    item.infoLabels['tmdb_id'] = tmdb_result['id']
    if tipo_busqueda == 'movie':
        item.contentType = 'movie'
        item.contentTitle = tmdb_result['title']
        item.title = tmdb_result['title']
    else:
        item.contentType = 'tvshow'
        item.contentSerieName = tmdb_result['name']
        item.title = tmdb_result['name']
    if 'thumbnail' in item.__dict__: item.__dict__.pop('thumbnail')
    if 'fanart' in item.__dict__: item.__dict__.pop('fanart')
    # ~ logger.debug(item)
...
def _seleccionar_un_resultado(results, tipo_busqueda):
    import xbmcgui
    opciones = []
    for res in results:
        logger.info(res)
        if tipo_busqueda == 'movie':
            ...
            if 'release_date' in res and res['release_date'] != '':
                titulo += ' (%s)' % res['release_date'][:4]
            if res['title'] != res['original_title']:
                titulo += ' [%s]' % res['original_title']
        else:
            titulo = res['name']
            if 'first_air_date' in res and res['first_air_date'] != '':
                titulo += ' (%s)' % res['first_air_date'][:4]
            if len(res['origin_country']) > 0:
                titulo += ' [%s]' % ','.join(res['origin_country'])
            if res['name'] != res['original_name']:
                titulo += ' [%s]' % res['original_name']

        it = xbmcgui.ListItem(titulo, res['overview'])
...
def find_and_set_infoLabels(item):
    ...
    if item.contentType == "movie":
        ...
        tipo_busqueda = "tv"
        tipo_contenido = "Enlace repetido"
    ...
    if len(results) > 1: # Diálogo para escoger resultado
        ret = _seleccionar_un_resultado(results, tipo_busqueda)
        if ret > -1: # Si no se ha cancelado asignar resultado
            tmdb_result = results[ret]
        # ~ from platformcode import platformtools
...
def get_genres(tipo):
    genres = Tmdb(tipo=tipo)

    return genres.dic_generos['es']

def get_discover(tipo, genero='', page=1):
    url = 'discover/' + tipo
    listado = Tmdb( discover = {'url': url, 'with_genres': genero, 'page': page, 'language': 'es'} )
...
def get_person(texto_buscado, page=1):
    listado = Tmdb( tipo='person', texto_buscado=texto_buscado, page=page )

    return listado.get_list_resultados()

def get_person_credits(person_id, castandcrew='cast'):
    listado = Tmdb( person_credits = { 'person_id': person_id, 'castandcrew': castandcrew } )
...
#            (opcional) texto_alternativo: (str) Texto o parte del texto a buscar si no se encuentra nada con texto_buscado.
#        Return:
...
#           identificador id_Tmd en la web themoviedb.org.
#
...
#           tipo: ("movie" o "tv") Tipo de resultado buscado peliculas o series. Por defecto "movie" (opcional)
#           idioma_busqueda: (str) codigo del idioma segun ISO 639-1
#       Return:
#           Esta llamada devuelve un objeto Tmdb que contiene el resultado de buscar una pelicula o serie con el
#           identificador 'external_id' de la web referenciada por 'external_source' en la web themoviedb.org.
#
#   Metodos principales:
...
class Tmdb(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.texto_alternativo = kwargs.get('texto_alternativo', '')

        self.busqueda_id = kwargs.get('id_Tmdb', '')
        self.busqueda_texto = re.sub('\[\\\?(B|I|COLOR)\s?[^\]]*\]', '', self.texto_buscado).strip()
        self.busqueda_alternativo = re.sub('\[\\\?(B|I|COLOR)\s?[^\]]*\]', '', self.texto_alternativo).strip()
        ...
        self.person_credits = kwargs.get('person_credits', {})
        # Reellenar diccionario de generos si es necesario
        if (self.busqueda_tipo == 'movie' or self.busqueda_tipo == "tv") and \
                (self.busqueda_idioma not in Tmdb.dic_generos or self.busqueda_tipo not in Tmdb.dic_generos[self.busqueda_idioma]):
        ...
        if self.busqueda_id:
            ...
            # Busqueda por texto
            res = self.__search(page=self.page)
            # Si no hay resultados y hay un texto alternativo probar con él
            if res == 0 and self.busqueda_alternativo not in ['', self.busqueda_texto]:
                logger.debug('Volver a buscar %s con título alternativo %s' % (self.busqueda_texto, self.busqueda_alternativo))
                self.busqueda_texto = self.busqueda_alternativo
        ...
        elif self.person_credits:
            self.__person_credits()
    ...
    def get_json(url):
        ...
        except WebErrorException as ex:
            logger.info('Falla el acceso a TMDB para %s' % url)
            dict_data = {}
    ...
    def rellenar_dic_generos(cls, tipo='movie', idioma='es'):
        ...
        if tipo not in cls.dic_generos[idioma]:
            ...
            url = ('https://api.themoviedb.org/3/genre/%s/list?api_key=%s&language=%s'
                   % (tipo, Tmdb.api_key, idioma))
            try:
    ...
    def __by_id(self, source='tmdb'):
        ...
        if self.busqueda_id:
            if source == "tmdb":
                ...
                #   &append_to_response=images,videos,external_ids,credits&include_image_language=es,null
                url = ('https://api.themoviedb.org/3/%s/%s?api_key=%s&language=%s'
                       '&append_to_response=images,videos,external_ids,credits&include_image_language=%s,null' %
                       (self.busqueda_tipo, self.busqueda_id, Tmdb.api_key, self.busqueda_idioma, self.busqueda_idioma))
                ...
                url = ('https://api.themoviedb.org/3/find/%s?external_source=%s&api_key=%s&language=%s' %
                       (self.busqueda_id, source, Tmdb.api_key, self.busqueda_idioma))
                buscando = "%s: %s" % (source.capitalize(), self.busqueda_id)

            logger.info("[Tmdb.py] Buscando %s:\n%s" % (buscando, url))
            resultado = self.get_json(url)
            if not isinstance(resultado, dict):
                resultado = eval(resultado.decode('utf-8'))
    ...
    def __search(self, index_results=0, page=1):
        ...
        if self.busqueda_texto:
            # ~ buscando = self.busqueda_texto.replace(' ', '%20').replace('&', '%26')
            import urllib
            buscando = quote(self.busqueda_texto)
            # https://api.themoviedb.org/3/search/movie?api_key=a1ab8b8669da03637a4b98fa39c39228&query=superman&language=es
            # &include_adult=false&page=1
            url = ('https://api.themoviedb.org/3/search/%s?api_key=%s&query=%s&language=%s&include_adult=%s&page=%s' %
                   (self.busqueda_tipo, Tmdb.api_key, buscando,
                    self.busqueda_idioma, self.busqueda_include_adult, page))

            if self.busqueda_year:
                if self.busqueda_tipo == 'tv': url += '&first_air_date_year=%s' % self.busqueda_year
                else: url += '&year=%s' % self.busqueda_year

            logger.info("[Tmdb.py] Buscando %s en pagina %s:\n%s" % (self.busqueda_texto, page, url))
            resultado = self.get_json(url)
            if not isinstance(resultado, dict):
                resultado = ast.literal_eval(resultado.decode('utf-8'))
            ...
            # ~ logger.info("[Tmdb.py] Resultados: %d total_results: %s total_pages: %s" % (len(resultado["results"]), total_results, total_pages))
        ...
        if results:
            ...
            if self.busqueda_tipo == 'person': self.result = self.results[index_results]
            else: self.result = ResultDictDefault(self.results[index_results])
    ...
    def __person_credits(self):
        person_id = self.person_credits.get('person_id', '')
        castandcrew = self.person_credits.get('castandcrew', '')

        url = ('https://api.themoviedb.org/3/person/%s/combined_credits?api_key=%s&language=%s' % (person_id, Tmdb.api_key, self.busqueda_idioma))
        ...
        # ~ logger.info("[Tmdb.py] Cast: %d Crew: %d" % (len(resultado['cast']), len(resultado['crew'])))
        
        if castandcrew == 'cast':
            self.result = resultado['cast']
        elif castandcrew == 'crew':
            self.result = resultado['crew']
        else:
            self.result = resultado['cast'] + resultado['crew']

        # ~ self.result = sorted(self.result, key=lambda it: it['popularity'], reverse=True)
        
        # Ordenar por fechas y descartar pelis/series futuras
        for it in self.result:
            if 'release_date' in it: it['tosort'] = it['release_date']
            elif 'first_air_date' in it: it['tosort'] = it['first_air_date']
            else: it['tosort'] = ''
        
        today = time.strftime('%Y-%m-%d')
        self.result = filter(lambda it: it['tosort'] != '' and it['tosort'] <= today, self.result)
        self.result = sorted(self.result, key=lambda it: it['tosort'], reverse=True)
    ...
    def __list(self, index_results=0):
        ...
        type_search = self.lista.get('url', '')
        if type_search:
            params = []
            for key, value in self.lista.items():
                if key != "url":
                    params.append("&"+key + "=" + str(value))
            # https://api.themoviedb.org/3/movie/popolar?api_key=a1ab8b8669da03637a4b98fa39c39228&&language=es
            url = ('https://api.themoviedb.org/3/%s?api_key=%s%s'
                   % (type_search, Tmdb.api_key, ''.join(params)))
            ...
            if index_results >= len(results):
                logger.error("La busqueda de '%s' no dio %s resultados" % (type_search, index_results))
                return 0
    ...
    def get_sinopsis(self, idioma_alternativo=""):
        ...
        if 'id' in self.result:
            ...
            if ret == "" and str(idioma_alternativo).lower() != 'none':
                ...
                url = ('https://api.themoviedb.org/3/%s/%s?api_key=%s&language=%s' %(self.busqueda_tipo, self.busqueda_id, Tmdb.api_key, self.busqueda_idioma))
    ...
    def get_temporada(self, numtemporada=1):
        ...
        if not self.temporada.get(numtemporada, {}):
            ...
            url = "https://api.themoviedb.org/3/tv/%s/season/%s?api_key=%s&language=%s" \
                  "&append_to_response=credits" % (self.result["id"], numtemporada, Tmdb.api_key, self.busqueda_idioma)
            ...
            try:
                ...
                if 'episodes' not in self.temporada[numtemporada]: self.temporada[numtemporada]['episodes'] = {}
    ...
    def get_videos(self):
        ...
        if self.result['id']:
            if self.result['videos']:
                ...
                url = "https://api.themoviedb.org/3/%s/%s/videos?api_key=%s&language=%s" \
                      % (self.busqueda_tipo, self.result['id'], Tmdb.api_key, self.busqueda_idioma)
            ...
            if self.busqueda_idioma != 'en':
                url = "https://api.themoviedb.org/3/%s/%s/videos?api_key=%s" \
                      % (self.busqueda_tipo, self.result['id'], Tmdb.api_key)
