...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR %s]Configurador Buffer[/COLOR]' % (COLOR2)
			...
			header = '[COLOR %s]CURL Timeout/CURL Baja velocidad[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s][B]curlclienttimeout[/B] es el tiempo en segundos que tardara la conexion de libcurl en agotar el tiempo de espera y [B] curllowspeedtime [/ B] es el tiempo en segundos que libcurl considerara una conexion a velocidad minima. Para conexiones mas lentas, configurelo en 20.[/COLOR]" % COLOR2
			...
			header = '[COLOR %s]Factor de lectura del Buffer[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s]El valor de esta configuracion es un multiplicador del limite predeterminado. Si Kodi esta cargando un archivo bluray tipico a 36 Mbit/s, entonces un valor de 2 necesitara al menos 72 Mbit/s de ancho de banda de red. Sin embargo, a diferencia de la configuracion de la memoria RAM, puede aumentar este valor de manera segura por alto que sea, y Kodi no se bloqueara.[/COLOR]" % COLOR2
			...
			self.Button2 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B3, (W/2)-(BorderWidth*4), 30, '2: Solo Buffer de archivos de internet reales', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			...
			self.buttonWrite=xbmcgui.ControlButton(leftside,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Escribir Archivo",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
		...
		def updateCurrent(self, control):
			if control == self.videoCacheSize:
				...
				current = '[COLOR %s]Actual:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, int(self.currentVideo))
				...
				current = '[COLOR %s]Actual:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, int(self.currentCurl))
				...
				current = '[COLOR %s]Actual:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, int(self.currentRead))
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Este es el actual y activo [COLOR %s]AdvancedSettings.xml[/COLOR], Quieres borrarlo y continuar?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Borrar ajustes[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar Escritura[/COLOR][/B]")
				if choice == 0: return
	...
	class firstRun(xbmcgui.WindowXMLDialog):
		...
		def onInit(self):
			...
			self.controllist     = [self.trakt, self.debrid, self.login, 
									self.sources, self.myvideos, self.profiles, self.advanced, 
			...
			self.controlsettings = ['keeptrakt', 'keepdebrid', 'keeplogin',
									'keepsources', 'keepmyvideos', 'keepprofiles', 'keepadvanced',
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "No se ha detectado ninguna Build %s instalada.\n\nElige 'Build Menu' para instalar la Build STVA o 'Ignorar' para no volver a ver este mensaje.\nSi ya tienes una version anterior de %s pulsa ignorar e instala un parche." % (ADDONTITLE, ADDONTITLE)
		...
		def showdialog(self):
			self.getControl(self.image).setImage('https://stva.zriel.ga/wizard/imagenes/cfanart.jpg')
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "[B]ACTUALIZACION DISPONIBLE[/B] \n\nSe ha detectado una nueva version de tu Build o Parche:\n[COLOR %s]%s[/COLOR]\n\nVersion Instalada: v[COLOR %s]%s[/COLOR]\nUltima version Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendaciones:[/COLOR]\nSi has realizado cambios en la Build, guarda una copia\nde seguridad (Skin y Menu) antes de instalar una nueva version\n\nFijate bien si vas a instalar una Build o Parche y pulsa\nel boton de instalacion correspondiente" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
			self.msgcurrent = "Ejecutando la ultima version de la Build instalada:\n[COLOR %s]%s[/COLOR]\n\nVersion Instalada: v[COLOR %s]%s[/COLOR]\nUltima Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendaciones:[/COLOR]\nSi has realizado cambios en la Build, guarda una copia \nde seguridad (Skin y Menu) antes de instalar una nueva version.\n\nFijate bien si vas a instalar una Build o Parche y pulsa\nel boton de instalacion correspondiente" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
