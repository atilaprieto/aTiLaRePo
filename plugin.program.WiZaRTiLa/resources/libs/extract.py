...
def allWithProgress(_in, _out, dp, ignore, title):
	...
	for item in zin.infolist():
		...
		line2 += '[COLOR %s][B]Peso:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR]' % (COLOR2, COLOR1, wiz.convertSize(size), zipsize)
		...
		elif item.filename == 'userdata/Database/MyVideos116.db' and KEEPMYVIDEOS == 'true': skip = True
		...
		if skip == True: wiz.log("Skipping: %s" % item.filename, xbmc.LOGNOTICE)
			...
			try:
				...
				wiz.log('Error Extrayendo: %s(%s)' % (item.filename, str(e)), xbmc.LOGERROR)
				pass
